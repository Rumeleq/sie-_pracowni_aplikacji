...
const server = http.createServer(async (req, res) => {
    ...
    const query = url.parse("http://localhost:8080/default.html?year=2023&month=september", true)
    ...
    if (req_url === '/message' && method === 'POST')
    {
        ...
        req.on("end", async () => {
            ...
            await fs.promises.writeFile(`message-${Date.now().toString()}.txt`, message)
            res.statusCode = 302
